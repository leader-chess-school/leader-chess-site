...
рядом с базой школы под общим родителем `leader-chess-school/` (двери у
родителя нет — это просто папка).

Соседняя папка `../leader-llm-wiki/` — **база школы** (ученики, группы, деньги,
...
- Заявки с сайта не попадают в `../leader-llm-wiki/school/leads/` — только в телеграм.

## Граница

Свой волт у сайта не заведён — карточка системы лежит в базе школы,
`../leader-llm-wiki/vault/systems/site.md`; инженерное состояние и история —
`docs/STATE.md` здесь же. Сам в чужие волты не пиши без прямой просьбы.
